...
def save_images(original_images, watermarked_images, noised_images, epoch, folder, watermark=None,resize_to=None):
    images = original_images[:original_images.shape[0], :, :, :].cpu()
    watermarked_images = watermarked_images[:watermarked_images.shape[0], :, :, :].cpu()
    noised_images=noised_images[:watermarked_images.shape[0], :, :, :].cpu()
    ...
    filename2 = os.path.join(folder, 'epoch-{}-noised.png'.format(epoch))
    torchvision.utils.save_image(stacked_images, filename, original_images.shape[0], normalize=False)
    torchvision.utils.save_image(noised_images, filename2, original_images.shape[0], normalize=False)

    if watermark is not None:
        filename3 = os.path.join(folder, 'epoch-{}--watermark.png'.format(epoch))
        torchvision.utils.save_image(watermark, filename3, original_images.shape[0], normalize=False)

def reshape_watermark(watermark):
    size = watermark.size()
    if size[1]!=1:
        columns = int(math.sqrt(size[1]))
        watermark = torch.reshape(watermark,(size[0],1,columns,-1))
    return watermark
