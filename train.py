...
def train(model: Hidden,
    ...
    for epoch in range(train_options.start_epoch, train_options.number_of_epochs + 1):
        ...
        training_losses = defaultdict(AverageMeter)    #如果字典中不存在该关键字，则返回默认的固定值
        epoch_start = time.time()
        step = 1
        for image, _ in train_data:
            image = image.to(device)
            message = torch.Tensor(np.random.choice([0, 1], (image.shape[0], hidden_config.message_length))).to(device)   #8 30
        ...
        for image, _ in val_data:
            ...
            if first_iteration:
                ...
                message = utils.reshape_watermark(message)[:images_to_save, :, :, :]
                decoded_messages = torch.round(decoded_messages)
                decoded_messages = utils.reshape_watermark(decoded_messages)[:images_to_save, :, :, :]
