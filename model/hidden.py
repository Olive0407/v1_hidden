...
class Hidden:
    def __init__(self, configuration: HiDDenConfiguration, device: torch.device, noiser: Noiser, tb_logger):
        ...
        #true为跑redmark，false为hidden
        self.encoder_decoder = EncoderDecoder(configuration, noiser, False).to(device)  #encoder decoder noiser
        self.discriminator = Discriminator(configuration).to(device)
        self.optimizer_enc_dec = torch.optim.Adam(self.encoder_decoder.parameters())
        self.optimizer_discrim = torch.optim.Adam(self.discriminator.parameters())

        if configuration.use_vgg:    #false
    ...
    def train_on_batch(self, batch: list):
        ...
        #size=messages.size()
        #messages_image=torch.reshape(messages, (-1, 1, int(math.sqrt(size[1])), size[1]/(int(math.sqrt(size[1]))))).cuda(messages.get_device())
        ...
        with torch.enable_grad():
            ...
            # train on cover
            d_target_label_cover = torch.full((batch_size, 1), self.cover_label, device=self.device)   #（8,1）且其中值为1
            d_target_label_encoded = torch.full((batch_size, 1), self.encoded_label, device=self.device)  #（8,1）且其中值为0
            g_target_label_encoded = torch.full((batch_size, 1), self.cover_label, device=self.device)   #（8,1）且其中值为1
            ...
            #decoded_image=torch.reshape(decoded_messages, (-1, 1, int(math.sqrt(size[1])), size[1]/(int(math.sqrt(size[1]))))))
            ...
            if self.vgg_loss == None:
                ...
                enc_ssim=self.ssim(encoded_images, images)
            else:
            ...
            #dec_ssim=self.ssim(decoded_image,messages_image)
        ...
        bitwise_avg_err = np.sum(np.abs(decoded_rounded - messages.detach().cpu().numpy())) / (
                batch_size * messages.shape[1])   #计算比特错误率
        ...
        losses = {
            ...
            'discr_encod_bce': d_loss_on_encoded.item(),
             'enc_ssim':enc_ssim.item(),
            #'dec_ssim':dec_ssim.item()
        }
