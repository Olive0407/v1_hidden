...
def main():
    ...
    new_run_parser.add_argument('--data-dir', '-d', default='/home/audience/dataset/mscoco', type=str,
                                help='The directory where the data is stored.')
    new_run_parser.add_argument('--batch-size', '-b',default=8, type=int, help='The batch size.')
    new_run_parser.add_argument('--epochs', '-e', default=600, type=int, help='Number of epochs to run the simulation.')
    new_run_parser.add_argument('--name', default='hidden', type=str, help='The name of the experiment.')
    ...
    continue_parser.add_argument('--folder', '-f', default='/home/audience/Desktop/2021824hidden3/runs/hidden 2021.08.28--23-30-50', type=str,
                                 help='Continue from the last checkpoint in this folder.')
    continue_parser.add_argument('--data-dir', '-d', default='/home/audience/dataset/mscoco', type=str,
                                 help='The directory where the data is stored. Specify a value only if you want to override the previous value.')
    continue_parser.add_argument('--epochs', '-e', default=600, type=int,
    ...
    continue_parser.add_argument('--gpu', default=1,type=int)
    ...
    if args.command == 'continue':
        ...
        if args.data_dir is not None:
            train_options.train_folder = os.path.join(args.data_dir, 'train3')
            train_options.validation_folder = os.path.join(args.data_dir, 'val4')
        ...
        train_options = TrainingOptions(
            ...
            train_folder=os.path.join(args.data_dir, 'train3'),
            validation_folder=os.path.join(args.data_dir, 'val4'),
            runs_folder=os.path.join('.', 'runs'),
            start_epoch=start_epoch,
            experiment_name=args.name)

        noise_config = args.noise if args.noise is not None else []
        hidden_config = HiDDenConfiguration(H=args.size, W=args.size,   #size为128*128
                                            message_length=args.message,   #水印长度 30bit
                                            ...
                                            enable_fp16=args.enable_fp16  #false
                                            )
        config = redmarkjpegconfig(H=args.size, W=args.size,  # size为128*128
                                batch_size=args.batch_size
                                            )

        this_run_folder = utils.create_folder_for_run(train_options.runs_folder, args.name)  #创建runs文件夹
        with open(os.path.join(this_run_folder, 'options-and-config.pickle'), 'wb+') as f:
            pickle.dump(train_options, f)   #将对象序列转换为二进制对象，写入文件
    ...
    model = Hidden(hidden_config, device, noiser, tb_logger)   #将攻击层写成类传入主网络
